---
name: "resume-tailor"
description: "Generate a folder with a .tex and .txt based on Overleaf premade resume pulled through set-up MCP Connector. The tailor judges which resume is best tailored to the job and what are the relevant changes to make based on the given information"
---
 
## Overview
 
Start by finding the reference resumes with the connector to Overleaf based in C:\Users\yanng\Documents\CV\Overleaf-mcp, for jobs in Montreal use the " fr" versions. Here are a short description of the resumes you can find based on their project_id:
...
- Create a folder named 'company name' in C:\Users\yanng\Documents\CV and create all subsequent files in it.
...
- Create a cover letter in a word doc name 'Cover Letter + company name' (respect Further Guidelines), if the job is in Montreal write it ENTIRELY in french and name it 'Lettre de Motivation + nom de la compagnie':
Structure
- TemplateHeader: Your contact info, date, and recipient info (for email use yanngiffaux@yahoo.com).
...
- Update 'Tracker.xlsx' at path 'C:\Users\yanng\Documents\CV' by searching for the Job name in column A and changing the same row's column D to created, follow Tracker Rules.
Resume Rules
...
- My location information should be Montreal if the job is in Québec and Ottawa if in Ontario.
- My email is yanngiffaux@yahoo.com.
